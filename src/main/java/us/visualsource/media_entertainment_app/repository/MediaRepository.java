...
public interface MediaRepository extends JpaRepository<Media, UUID> {
    ...
    @Query(value = "SELECT * FROM media m JOIN bookmarks b ON m.uuid = b.media_id WHERE b.media_type = :type AND m.name LIKE %:query%",
            nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM media m JOIN bookmarks b ON m.uuid = b.media_id WHERE b.owner_id = :owner AND m.name LIKE %:query%",
            nativeQuery = true)
    List<Media> searchBy(@Param("query") String query, @Param("owner") Long owner);
