    ...
    return (
        <div className="relative min-h-min h-full">
            <div className='relative shadow w-full h-[8rem] md:h-36 lg:h-[11.5rem]'>
                <button onClick={() => mutation.mutateAsync({ state: bookmarked, id })} className="absolute top-3 right-3 md:right-4 h-10 w-10 md:h-8 md:w-8 bg-neutral-700/65 hover:bg-neutral-800/95 transition rounded-full flex items-center justify-center text-zinc-50"> {bookmarked ? <BookmarkCheck className="h-8 w-8 md:h-4 md:w-4" /> : <Bookmark className="h-5 w-5 md:h-4 md:w-4" />} </button>
